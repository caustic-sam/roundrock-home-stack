...
class RPiDiagnostics:
    ...
    def detect_hardware(self):
        ...
        # Detect cooling
        info['has_cooling'] = bool(
            self.run_command("grep -q fan /sys/class/thermal/cooling_device*/type 2>/dev/null; echo $?") == "0" or
            self.run_command("systemctl is-active fancontrol") == "active" or
            self.run_command("pgrep -f 'gpio.*fan'")
        )
...
if __name__ == "__main__":
    ...
    main()_info['has_ethernet']:
            eth_status = "Connected" if "state UP" in self.run_command("ip link show eth0 2>/dev/null") else "Disconnected"
        ...
        if gateway:
            network['gateway_reachable'] = self.run_command(f"ping -c 2 -W 3 {gateway} >/dev/null 2>&1; echo $?") == "0"
        
        network['internet_reachable'] = self.run_command("ping -c 2 -W 3 8.8.8.8 >/dev/null 2>&1; echo $?") == "0"
        network['dns_working'] = self.run_command("nslookup google.com >/dev/null 2>&1; echo $?") == "0"
    ...
    def get_ai_hat_info(self):
        ...
        # I2C devices
        i2c_info = []
        i2c_buses = self.run_command("ls /dev/i2c-* 2>/dev/null")
        for bus in i2c_buses.split():
            if bus:
                bus_num = bus.split('-')[-1]
                bus_devices = self.run_command(f"i2cdetect -y {bus_num} 2>/dev/null | grep -v '^     '")
                if bus_devices:
        ...
        # Power consumption
        power_files = self.run_command("ls /sys/class/hwmon/hwmon*/power1_input 2>/dev/null")
        if power_files:
            for power_file in power_files.split():
                if power_file:
                    power_uw = self.run_command(f"cat {power_file}")
                    if power_uw.isdigit():
...
<body>
    ...
    <div class="container">
        ...
        if self.hardware_info['has_ethernet']:
            ...
            html_content += f"""
                <div class="hw-card {eth_class}">
                    ...
                    {'<div>IP: ' + eth_ip + '</div>' if eth_ip else ''}
                </div>
            """
        ...
        if self.hardware_info['has_cooling']:
            ...
            temp_class = "success"
            if temp != 'N/A' and temp.replace("'C", "").replace("°C", "").replace(".", "").isdigit():
                temp_val = float(temp.replace("'C", "").replace("°C", ""))
                if temp_val > 70:
                    temp_class = "warning"
                if temp_val > 80:
                    temp_class = "error"
        ...
        # Storage card
        storage_usage = self.run_command("df -h / | awk 'NR==2{printf \"%s used of %s (%.0f%%)\", $3, $2, ($3/$2)*100}'")
        html_content += f"""
                <div class="hw-card available">
                    <h3>💾 Storage</h3>
        ...
        html_content += f"Throttling Status: {power_status.get('throttled', 'N/A')}\n"
        html_content += "\n".join(power_status.get('recent_events', []))
...
{ai_info.get('power_draw', 'Power monitoring not available')}
                    </pre>
                </div>
            </div>
            """
...
{logs.get('power_events', 'No power/thermal events')}
                    </pre>
                </div>
            </div>
        """
